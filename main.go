...
var hcwClient *hcrpcclient.Client
...
func requestFunds(w http.ResponseWriter, r *http.Request) {
	...
	hcamount, err := hcutil.NewAmount(amount)
	if err != nil {
		sendReply(w, r, tmpl, testnetFaucetInformation, err)
		return
	}

	resp, err := hcwClient.SendToAddress(address, hcamount)
	if err != nil {
...
func main() {
	...
	hcwCerts, err := ioutil.ReadFile(cfg.WalletCert)
	if err != nil {
	...
	connCfgDaemon := &hcrpcclient.ConnConfig{
		...
		Certificates: hcwCerts,
		DisableTLS:   false,
	}
	hcwClient, err = hcrpcclient.New(connCfgDaemon, nil)
	if err != nil {
